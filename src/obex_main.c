...
#define UPUSH_APPNAME "ussp-push v0.9"
...
void handle_put_error(obex_t * handle, obex_object_t * object, int obex_rsp) {
  client_context_t *gt = OBEX_GetUserData(handle);

  if (obex_rsp == OBEX_RSP_SUCCESS) {
    //BTERROR("Reciving file OK\n");
    gt->transfer_error= 0;
  } else if (obex_rsp == OBEX_RSP_FORBIDDEN) {
    //BTERROR("Reciving file failed\n");
    gt->transfer_error= 1;
  } else {
    // char* obex_rsp_response= OBEX_GetResponseMessage (handle, obex_rsp);
    //BTERROR("Reciving file error: %s\n", obex_rsp_response);
  }
}
...
void request_done(obex_t * handle, obex_object_t * object, int obex_cmd, int obex_rsp)
{
	...
	BTDEBUG("Command (%02x) has now finished, rsp: %02x %s\n", obex_cmd, obex_rsp, obex_rsp_response);
	...
	case OBEX_CMD_PUT:
		/* put_client(handle, object, obex_rsp); */
                BTDEBUG("put\n");
                handle_put_error(handle, object, obex_rsp);
		gt->clientdone = 1;
		break;
...
int obex_push(int devid, void *addr, char *path, char *remote, int timeout)
{
	...
	if (buf == NULL) {
		//BTERROR("Can't find file %s\n", name);
                // error when trying to find the file
		return 1;
	}
	...
	err = handle_response(handle, BT_SERVICE);
        ...
        transfer_error= gt->transfer_error;
	obex_disconnect(handle);
	free(buf);
	free(pathc);

        if (err != 0) {
          // error when reciving the confirmation
          return 4;
        }
        if (transfer_error == 1) {
          // the user choosed to not accept the file
          return 5;
        }
        return 0;
}
...
int obex_push_python(char *addr, char *path, char *remote, int timeout)
{
 int devid= 0;
 int ret;
 bt_debug = 0;
 BTDEBUG("pushing file %s\n", path);
 ret= obex_push(devid, (void *) addr, path, remote, timeout);
 return ret;
}
...
int main(int argc, char **argv)
{
	...
	BTDEBUG("pushing file %s\n", argv[i + 1]);
	if (obex_push_python(argv[i], argv[i + 1], argv[i + 2], timeout) != 0) {
