...
int cobex_handle_input(obex_t * handle, void *userdata, int timeout)
{
	...
	struct timeval time;
	fd_set fdset;
	int ret, err_counter=0, stop_counter=60*20;
	int16_t expectlen;
	...
	do {
		...
		if (ret == 0)
                {
                        /* dont use much cpu when waiting for the remote host */
                        //printf("wait %d\n", err_counter);
                        sleep(1);
                        err_counter++;
                        if (err_counter > stop_counter)
                        {
                              BTDEBUG("Connection timeout %d seconds.\n", stop_counter);
                              //printf("timeout %d\n", err_counter);
                              return -1;
                        }
			len = 0;
